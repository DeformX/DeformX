...
"""
Isaac Sim 4.5 Standalone – depth-only capture
...
- Renders depth only from a specified camera

Run:
  /home/robot/isaacsim/python.sh test_drop_multi_diff_rods_depth_only.py
"""
...
BACKGROUND_USDS = [
    (
        "/home/robot/Workspace/CosseratX/Dataset_generator_datacenter/data/Datacenter_NVD@10012/Assets/DigitalTwin/Assets/Datacenter/Facilities/Stages/Data_Hall/DataHall_Full_01.usd",
        1.0,
    ),
    (
        "/home/robot/Workspace/CosseratX/Dataset_generator_datacenter/data/data_center_scene.usdc",
        100.0,
    ),
]
...
RENDER_OUTPUT_DIR = "/home/robot/Workspace/CosseratX/Dataset_generator_datacenter/data/renders_depth_only"
...
def load_npz_data(npz_path):
    ...
    if "director" in data.files:
        ...
        print("[INFO] No 'director' in NPZ — will compute from positions")
...
def build_stage(num_wires):
    ...
    for i in range(num_wires):
        ...
        xformable.AddTranslateOp().Set(offsets[i])

        if POS_SCALE != 1.0:
            xformable.AddScaleOp().Set(Gf.Vec3f(POS_SCALE, POS_SCALE, POS_SCALE))
...
# ============================================================
def render_depth_frames(stage, wire_roots, positions, time_arr):
    import omni.replicator.core as rep
    from PIL import Image

    T = positions.shape[0]
    if T < 2:
        raise RuntimeError("Need at least 2 frames in NPZ to infer fps from time array.")
    ...
    print(f"[INFO] Capturing {len(frames)} depth frame(s) → {depth_dir}")
    ...
    for frame in frames:
        ...
        # Some Isaac builds return HxW, some HxWx1
        if depth.ndim == 3 and depth.shape[-1] == 1:
            depth = depth[..., 0]

        npy_path = os.path.join(depth_dir, f"{frame:04d}.npy")
        png_path = os.path.join(depth_dir, f"{frame:04d}.png")
        ...
        if np.any(valid):
            ...
            if dmax - dmin < 1e-8:
                ...
                vis[valid] = (depth[valid] - dmin) / (dmax - dmin)
        ...
        print(f"  [DEPTH] Saved: {png_path}")

    print(f"[INFO] Depth render complete → {depth_dir}")
...
def main():
    ...
    render_depth_frames(stage, wire_roots, positions, time_arr)

...
Segmentation behavior in this version:
- RGB: render full scene
- Instance segmentation: temporarily hide everything except /World/Wire_i
- Depth: restore full scene before capture
...
# CONFIG
# ============================================================
CABLE_ASSET_PATH = "/home/robot/Workspace/CosseratX/Dataset_generator_datacenter/data/cables/data cernter cable_grey.usdc"
NPZ_PATH = "/home/robot/Workspace/CosseratX/Dataset_generator_datacenter/data/npz_file/mid.npz"
...
def load_npz_data(npz_path):
    ...
    if "director" in data.files:
        ...
        if director.shape[0] != T or director.shape[1] != N:
            raise RuntimeError("director T/N mismatch")
        D = director.shape[4]
        if K != D + 1:
...
def build_rod_dir_from_positions(pos):
    ...
    up1 = np.array([0.0, 0.0, 1.0])[None, None, :, None]
    up2 = np.array([0.0, 1.0, 0.0])[None, None, :, None]
...
def add_semantic_labels(stage, wire_roots):
    ...
    instance segmentation annotator works at the mesh/prim level.
    """
    from pxr import Sdf
...
# ============================================================
# Visibility helpers for segmentation-only cable rendering
# ============================================================
def _is_under_any_root(path_str, roots):
    for root in roots:
...
def snapshot_visibility(stage):
    """
    Save authored visibility for all imageable prims.
    Returns dict[path_str] = authored_value_or_None
    """
    state = {}
    for prim in stage.Traverse():
        ...
        if attr.HasAuthoredValue():
            state[prim.GetPath().pathString] = attr.Get()
        else:
            state[prim.GetPath().pathString] = None
    return state


def restore_visibility(stage, state):
    """
    Restore visibility snapshot created by snapshot_visibility().
    """
    for prim in stage.Traverse():
        imageable = UsdGeom.Imageable(prim)
        if not imageable:
...
def set_visibility_for_seg(stage, visible_roots, also_keep_paths=None):
    """
    For segmentation pass:
    - keep cable roots visible
    - optionally keep some extra paths visible (e.g. camera, world)
    - hide everything else that is imageable
    """
    keep_paths = set(also_keep_paths or [])
    ...
    for prim in stage.Traverse():
        ...
        keep_visible = (
            _is_under_any_root(path_str, visible_roots)
            or path_str in keep_paths
        )
...
def render_frames(stage, wire_roots, positions, time_arr):
    ...
    RGB renders full scene.
    Segmentation temporarily hides everything except the cables.
    """
    ...
    # Snapshot once per frame, because some prims may change later
    for fi, frame in enumerate(frames):
        ...
        # ---- Instance Segmentation (only wires visible) ----
        print(f"  [RENDER] Frame {frame}: writing instance_id_segmentation (cables only)...")
        vis_state = snapshot_visibility(stage)
        try:
            set_visibility_for_seg(
                stage,
                visible_roots=wire_roots,
                also_keep_paths={"/World", "/World/RenderCamera"},
            )
            ...
            restore_visibility(stage, vis_state)
            for _ in range(2):
                simulation_app.update()

        # ---- Depth (full scene restored) ----

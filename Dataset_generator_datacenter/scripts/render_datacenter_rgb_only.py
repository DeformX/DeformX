...
"""
Isaac Sim 4.5 Standalone – with RGB capture
...
# Number of ray-tracing subframes for each capture (higher = cleaner, slower)
RT_SUBFRAMES = 256
...
# ============================================================
# Render frames: RGB only
# ============================================================
def render_frames(stage, wire_roots, positions, time_arr):
    """
    Render RGB only using BasicWriter.
    Each frame writes to a temporary sub-directory, then the image is
    renamed to rgb/<frame_index>.png so that "all" mode never overwrites.

    Ghosting fix: reset RTX path-tracer accumulation buffer between frames
    via carb settings, and disable temporal denoiser / temporal AA.
    """
    import glob
    import shutil
    import carb
    ...
    # Use the stage's actual TimeCodesPerSecond for timeline seeking.
    # This MUST match what author_animation() set, NOT a hardcoded value.
    tcps = stage.GetTimeCodesPerSecond()
    print(f"[INFO] Stage TimeCodesPerSecond = {tcps}")

    # ---- RTX renderer settings to eliminate ghosting ----
    settings = carb.settings.get_settings()

    # Disable temporal AA (causes ghosting across frame seeks)
    settings.set_int("/rtx/post/aa/op", 0)

    # Force path-tracer accumulation reset every frame (not only on movement)
    settings.set_bool("/rtx/resetPtAccumOnlyWhenMovement", False)

    # Disable DLSS / temporal denoiser if present
    settings.set_bool("/rtx/post/dlss/enabled", False)
    settings.set_int("/rtx/post/denoiser/method", 0)          # 0 = off
    settings.set_bool("/rtx/directLighting/deferredLighting", False)
    settings.set_bool("/rtx/indirectDiffuse/enabled", True)

    # Path-tracer: 1 spp per iteration, high total for clean single-frame
    settings.set_int("/rtx/pathtracing/spp", 1)
    settings.set_int("/rtx/pathtracing/totalSpp", 128)

    print("[INFO] RTX temporal AA / denoiser disabled for clean per-frame capture")
    ...
    print(f"[INFO] Capturing {len(frames)} RGB frames → {rgb_dir}")
    ...
    def flush_renderer(n_iters):
        """Force-reset accumulation buffer then run n_iters updates."""
        # Toggle a flag to force the path-tracer to drop its history
        settings.set_bool("/rtx/resetPtAccum", True)
        simulation_app.update()
        settings.set_bool("/rtx/resetPtAccum", False)
        for _ in range(n_iters):
            simulation_app.update()

    for fi, frame in enumerate(frames):
        # Seek timeline to target frame (timecode = frame, time = frame / tcps)
        timeline.set_current_time(frame / tcps)

        # Flush renderer: reset accumulation and let it reconverge
        flush_renderer(SETTLE_FRAMES)

        # Write RGB to a per-frame temp directory to avoid overwrite
        tmp_dir = os.path.join(RENDER_OUTPUT_DIR, f"_tmp_rgb_{frame:06d}")
        os.makedirs(tmp_dir, exist_ok=True)

        print(f"  [RENDER] Frame {frame}/{T-1}: writing RGB...")
        ...
        # Move the generated image(s) to rgb/<frame>.png
        found = sorted(glob.glob(os.path.join(tmp_dir, "**", "*.png"), recursive=True))
        if found:
            dst = os.path.join(rgb_dir, f"{frame:06d}.png")
            shutil.move(found[0], dst)
            print(f"  [RENDER] Saved → {dst}")
        else:
            print(f"  [WARN] No RGB image generated for frame {frame}")

        # Clean up temp directory
        shutil.rmtree(tmp_dir, ignore_errors=True)

        print(f"  [RENDER] Frame {frame} complete ({fi+1}/{len(frames)})")

    print(f"[INFO] Render complete → {rgb_dir}")

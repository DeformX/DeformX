...
parser.add_argument("--num_nodes", type=int, default=42, help="Wire node count (must be >= 2).")
parser.add_argument("--wire_length", type=float, default=2.09, help="Wire total length in meters.")
...
def main():
    ...
    q_last_wxyz = np.array([0.0, 0.0, 0.0, 0.0], dtype=np.float32)  # 180deg about Y
    ...
    first_center[0] -= float(2.0 * link_half)
    ...
    try:
        while simulation_app.is_running():
            ...
            first_view.set_world_poses(positions=first_center.reshape(1, 3))
            last_view.set_world_poses(positions=last_center.reshape(1, 3))

            # Keep inspector translate in sync.
            set_translate_op(stage, first_link, first_center)
            set_translate_op(stage, last_link, last_center)
            ...
            if steps == 1:
                ...
                e0 = c0 + d0 * float(link_half)  # free end position (local +Z)
                err = np.linalg.norm(e0 - np.asarray(first_node, dtype=np.float64))
                chain_dir = -d0
                print(
                    f"[Debug] link0_free_end=({e0[0]:+.4f},{e0[1]:+.4f},{e0[2]:+.4f}) "
